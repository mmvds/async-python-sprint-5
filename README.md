...
Скорее всего не получилось запустить из-за позднего старта postgresql, добавил healthcheck для контейнера
Т.к. при инициализации класса FilesStorageService вызывается db_init(), которая создает базу даже если alembic не запускался
Миграции добавил, также добавил запуск миграций во время запуска контейнера
Сервис через nginx работает на localhost:80/api/v1/
Также можно запустить тесты из service контейнера
```
